...
function categoriesMatchTolerant(catA, catB) {
  ...
  const normA = normalizeChestName(catA);
  const normB = normalizeChestName(catB);
  
  // Exakte Übereinstimmung
  if (normA === normB) return true;
  
  // Spezial-Behandlung für ähnliche Kategorien
  const tartarosAliases = ['chests of tartaros', 'tartaros chests', 'tartaros', 'crypt'];
  const isATartaros = tartarosAliases.some(alias => normA.includes(alias));
  const isBTartaros = tartarosAliases.some(alias => normB.includes(alias));
  
  return isATartaros && isBTartaros;
}
...
export function calculatePlayerNorms({ playersArr, resultsArr, chestMappings, normsArr, ignoreList, periodsArr, currentPeriodId }) {
  ...
  function isIgnoredChest(chest) {
    ...
    for (const ignore of ignoreList) {
      if (ignore.Name && ignore.Name.trim() !== "" && 
        String(ignore.Name).trim() !== String(chest.Name ?? chest.name ?? "").trim()
      ) continue;
      if (
        ignore.Level &&
