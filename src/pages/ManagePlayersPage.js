...
export default function ManagePlayersPage({ t, setCurrentPage }) {
  ...
  useEffect(() => {
    async function findMissingClanmates() {
      ...
      const allMainNames = new Set(playersArr.map(p => (p.name || '').trim().toLowerCase()));
      ...
      const missing = Array.from(allClanmates).filter(name => {
        if (!name) return false;
        const mapped = mapToMainName(playersArr, name).trim().toLowerCase();
        return !allMainNames.has(mapped);
      });
    ...
    <div className="min-h-screen flex flex-col bg-gray-900 text-white p-4 pb-8 relative">
      ...
      <div className="flex w-full max-w-7xl mx-auto">
        ...
        >
          ...
          {[...troopStrengths]
            .sort((a, b) => {
              // Versuche numerisch zu sortieren, sonst fallback auf String
